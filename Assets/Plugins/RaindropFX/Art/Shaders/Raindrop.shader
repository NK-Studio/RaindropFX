Shader "Hidden/Universal Render Pipeline/RainDropFX"
{
    HLSLINCLUDE
    #pragma multi_compile_local_fragment _ _FogEnable
    #pragma multi_compile_local_fragment _ _BlurEnable
    ...
    float3 ApplyBoxBlur(float2 uv, float focus, int sampleCount)
    {
        ...
        // 샘플링을 x축과 y축 방향으로 모두 수행
        for (int j = 0; j < sampleCount; j++)
        {
            // x, y 방향으로 오프셋을 주어 여러 방향에서 샘플링
            float2 offset = float2(j, 0) / _ScreenParams.y; // x방향 오프셋
            float2 offsetY = float2(0, j) / _ScreenParams.y; // y방향 오프셋

            // 샘플링을 x, y 방향 모두 진행
            float4 sampledColorX = _BlitTexture.SampleLevel(sampler_TrilinearClamp, uv + offset, focus);
            float4 sampledColorY = _BlitTexture.SampleLevel(sampler_TrilinearClamp, uv + offsetY, focus);

            accumulatedColor += sampledColorX.rgb;
            accumulatedColor += sampledColorY.rgb;
        }

        // 샘플 개수로 나누어 평균을 구합니다.
        return accumulatedColor / (sampleCount * 2); // x, y 방향 샘플링을 고려해 2배로 나눔
    }

    float4 Frag(Varyings i) : SV_Target
    {
        // 화면 좌표계 조정
        float2 normalizedUV = (i.texcoord * _ScreenParams.xy - 0.5 * _ScreenParams.xy) / _ScreenParams.y;
...
layer2Intensity);
        ...
        // 텍스처 좌표 생성
        float4 textureCoords = float4(uv.x + normalOffset.x, uv.y + normalOffset.y, 0, focusValue);

        // 기본 텍스처 샘플링
        float4 baseColor = _BlitTexture.SampleLevel(sampler_PointClamp, textureCoords.xy, 0);

        // 초기 blendedColor는 baseColor로 설정
        float4 blendedColor = baseColor;

        // Blur 효과가 활성화된 경우
        // #if defined(_BlurEnable)
